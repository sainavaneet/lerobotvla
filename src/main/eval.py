...
class Gr00tRobotInferenceClient:
    ...
    def get_action(self, img1, img2, state):
        obs_dict = {
            "video.main": img1[np.newaxis, :, :, :],
            "video.secondary_0": img2[np.newaxis, :, :, :],
        ...
        with robot.activate():
            for i in tqdm(range(ACTIONS_TO_EXECUTE), desc="Executing actions"):
                ...
                for i in range(ACTION_HORIZON):
                    ...
                    print("concat_action_deg", concat_action_deg)
                    time.sleep(0.02)
                    ...
                    #     cv2.imwrite(f"eval_images/img_{image_count}.jpg", img1)
                    #     cv2.imwrite(f"eval_images/img_{image_count}_secondary.jpg", img2)
                    #     image_count += 1
